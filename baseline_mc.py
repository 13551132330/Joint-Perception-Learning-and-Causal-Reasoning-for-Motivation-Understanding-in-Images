...
def run_training():
  ...
  with tf.Graph().as_default():
    ...
    train_writer = tf.summary.FileWriter('./visual_logs/baseline_mc_visual_logs/train', sess.graph)
    test_writer = tf.summary.FileWriter('./visual_logs/baseline_mc_visual_logs/test', sess.graph)
    for step in xrange(FLAGS.max_steps+1):
      ...
      if (step) %50 == 0 or (step + 1) == FLAGS.max_steps:
        ...
        summary,mc_acc,mc_loss_value = sess.run(
                        [merged,mc_accuracy,mc_loss],
                        feed_dict={   
                              images_placeholder: train_images,
                              mc_labels_placeholder: train_mc_labels,
                              keep_pro : 1
      ...
      if step > 1 and step % 5000 == 0:
        checkpoint_path = os.path.join('./models/baseline_mc_models', 'model.ckpt')
