...
FLAGS = flags.FLAGS
gpu_num = 1
pre_model_save_dir = './models/baseline_sc_models'
...
def run_training():
  ...
  with tf.Graph().as_default():
    ...
    predictions = tf.nn.top_k(tf.nn.softmax(sc_logits),5)
    #ac_accuracy = tower_acc(ac_logits, ac_labels_placeholder)
    sc_accuracy = topk_acc(tf.nn.softmax(sc_logits), sc_labels_placeholder,5)
  ...
  for step in xrange(FLAGS.max_steps):
    ...
    val_actions,val_images,val_ac_labels,val_sc_labels,val_mc_labels, next_strat_pos, _= input_data(
                        ...
                        shuffle=False)
    predict,sc_acc,sc_loss_value = sess.run(
                       [predictions,sc_accuracy,sc_loss],
                        feed_dict={
...
def main(_):
  ...
  for i in range(len(label_list)):
    ...
    if tmp_sc_label in label_list[i]:
        true_list[int(tmp_sc_label)]+=1
  for i in range(100):
  ...
  for i in range(len(label_list)):
    ...
    tmp_sc_label = cluster_100['sc'][int(line_num)]
    tmp_mc_label = cluster_256['mc'][int(line_num)]
    f=open('./test/scene/%d.txt'%int(tmp_sc_label),'a+')
    f.write(str(int(line_num)+1)+'\t'+dirname+'\t'+scene+'\t'+'groud_truth:'+str(tmp_sc_label)+'\t'+'predict:'+str(label_list[i])+'\n')    
  '''
