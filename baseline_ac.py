...
def run_training():
  ...
  with tf.Graph().as_default():
    ...
    opt_ac = tf.train.AdamOptimizer(learning_rate)
    ...
    for gpu_index in range(0, gpu_num):
      with tf.device('/gpu:%d' % gpu_index):
        ...
        grads1 = opt_ac.compute_gradients(ac_loss, varlist1)
    ...
    apply_gradient_ac = opt_ac.apply_gradients(grads1, global_step=global_step)
    ...
    train_writer = tf.summary.FileWriter('./visual_logs/baseline_ac_visual_logs/train', sess.graph)
    test_writer = tf.summary.FileWriter('./visual_logs/baseline_ac_visual_logs/test', sess.graph)
    for step in xrange(FLAGS.max_steps+1):
      ...
      if (step) %50 == 0 or (step + 1) == FLAGS.max_steps:
        ...
        summary,ac_acc,ac_loss_value = sess.run(
                        [merged,ac_accuracy,ac_loss],
                        feed_dict={   
                              images_placeholder: train_images,
                              ac_labels_placeholder: train_ac_labels,
                              keep_pro : 1
      ...
      if step > 1 and step % 5000 == 0:
        checkpoint_path = os.path.join('./models/baseline_ac_models', 'model.ckpt')
