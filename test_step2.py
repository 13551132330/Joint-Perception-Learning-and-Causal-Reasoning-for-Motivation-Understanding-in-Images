...
pre_model_save_dir = './models/step2_models'
gpu_num = 1
...
def run_training():
  ...
  for step in xrange(FLAGS.max_steps):
    ...
    val_actions,val_scenes,val_images,val_ac_labels,val_sc_labels,val_mc_labels,next_strat_pos, _= input_data(
                        filename='./list/test.list',
                        start_pos=next_strat_pos,
                        batch_size=FLAGS.batch_size * gpu_num,
                        shuffle=False)

    sc_predict,ac_predict,mc_predict,sc_acc,ac_acc,mc_acc,sc_loss_value,ac_loss_value,mc_loss_value = sess.run(
                        [sc_predictions,ac_predictions,mc_predictions,sc_accuracy,ac_accuracy,mc_accuracy,sc_loss,ac_loss,mc_loss],
                        feed_dict={
...
def main(_):
  ...
  for i in range(len(sc_label_list)):
    ...
    tmp_sc_label = cluster_100['sc'][int(line_num)]
    tmp_mc_label = cluster_256['mc'][int(line_num)]
    f=open('./test/step2/predict.txt','a+')
